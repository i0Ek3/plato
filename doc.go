...
//
//			1.2 语法分析：
//				将 1.1 中的 token 重新格式化为可以描述每个语法和其他语法相关联的一种表示，
//	         这也是我们常说的中间（表示）代码或者 ast。ast 是一种深度嵌套的对象，
//	         它以一种既易于使用又能告诉我们大量信息的方式来表示代码
//
...
//
//			这一步会对 1.2 中生成的 AST 进行操作，除了做一些修改以外，
//	     还会将其转换为其他语言，那具体是如何转换 AST 的呢？
//
//			首先，AST 中每个节点的结构其实是比较相似的，都有相同的属性或者类型，
//	     每个节点又是一个单独的 AST，根据 1.2 中的 AST，我们可以看到某个 AST 节点
//
//			其中，
//	     NumberLiteral 节点包含有以下属性：
//					{
//						type: 'NumberLiteral',
//						value: '2'
//					}
...
//					}
//
//			当我们转换 AST 时，我们可以对节点中的元素进行添加、删除、替换等操作，
//	     我们也可以添加、删除一个节点，或者基于该节点再新建一个新的节点
//
//			这里我们仅关注于创建一个目标语言的新节点，为了能够获取所有的 AST 节点，
//	     我们需要利用 dfs 来遍历它们，即深度优先遍历
//
...
//
//			简单来说，大多数编译器的代码生成就是消除转换表示中的 AST 和字符串化的代码，
//	     我们这里会使用刚才创建好的 AST 来进行转换
//
//
